...
The Negotiator is three agents chained by a single shared contract — the **Structured Job Spec**. Build the spec once, confirm it with the user, and reuse it *verbatim* on every call so every quote is for the exact same job.
...
│  2 · CALLER     │   parallel calls: Batch / Twilio / SIP
└─────────────────┘   ≥3 counterparty styles
    │  Quote[]  (itemized, comparable, fee-broken-down)
    ▼
┌─────────────────┐
│  3 · CLOSER     │  negotiate (leverage bids, push fees, red-flags) → rank → report
└─────────────────┘
